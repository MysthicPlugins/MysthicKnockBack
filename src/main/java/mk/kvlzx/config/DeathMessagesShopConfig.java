...
public class DeathMessagesShopConfig {
    ...
    private List<Integer> deathMessageSlots;
    private String deathMessageTitle;
    private List<String> deathMessageLore;
    ...
    // Death message items
    private Map<String, DeathMessageItem> deathMessageItems;
    ...
    public DeathMessagesShopConfig(MysthicKnockBack plugin) {
        this.plugin = plugin;
        configFile = new CustomConfig("death-messages-shop.yml", "config/menus", plugin);
    ...
    public void loadConfig() {
        ...
        // Death messages configuration
        deathMessageSlots = config.getIntegerList("menu.items.death_messages.slots");
        deathMessageTitle = config.getString("menu.items.death_messages.title");
        deathMessageLore = config.getStringList("menu.items.death_messages.lore");
        enchantedIfSelected = config.getBoolean("menu.items.death_messages.enchanted_if_selected");
        hideEnchants = config.getBoolean("menu.items.death_messages.hide_enchants");
        ...
        // Load death message items
        loadDeathMessageItems(config);
    ...
    private void loadRarityMaterials(FileConfiguration config) {
        ...
        ConfigurationSection materialsSection = config.getConfigurationSection("menu.items.death_messages.materials");
            ...
            for (String rarity : materialsSection.getKeys(false)) {
                ...
                if (raritySection != null) {
                    String selected = validateAndGetMaterial(config, 
                        "menu.items.death_messages.materials." + rarity + ".selected", "ENCHANTED_BOOK");
                    String unselected = validateAndGetMaterial(config, 
                        "menu.items.death_messages.materials." + rarity + ".unselected", "PAPER");
        ...
        if (!rarityMaterials.containsKey("EPIC")) {
            rarityMaterials.put("EPIC", new MaterialPair("ENCHANTED_BOOK", "BOOK"));
        }
    ...
    private void loadDeathMessageItems(FileConfiguration config) {
        deathMessageItems = new HashMap<>();
        ConfigurationSection deathMessageItemsSection = config.getConfigurationSection("death_message_items");
        
        if (deathMessageItemsSection != null) {
            for (String key : deathMessageItemsSection.getKeys(false)) {
                ConfigurationSection messageSection = deathMessageItemsSection.getConfigurationSection(key);
                if (messageSection != null) {
                    DeathMessageItem messageItem = new DeathMessageItem(
                        key,
                    ...
                    deathMessageItems.put(key, messageItem);
                }
            }
        }
    }
                        ...
                        if (materialName != null && !materialName.trim().isEmpty()) {
                            try {
                                ...
                                plugin.getLogger().severe("Invalid material detected: '" + materialName + 
                                    "' at 'menu.items.death_messages.materials." + rarity + "." + key + "'");
                                allValid = false;
                            }
                        }
                    }
    ...
    public void reload() {
        ...
        if (!validateAllMaterials()) {
            plugin.getLogger().warning("Invalid materials detected in death messages menu configuration. The menu will be loaded with default values for problematic materials.");
        }
    ...
    // Getters for death messages configuration
    public List<Integer> getDeathMessageSlots() { return new ArrayList<>(deathMessageSlots); }
    public String getDeathMessageTitle() { return deathMessageTitle; }
    public List<String> getDeathMessageLore() { return new ArrayList<>(deathMessageLore); }
    ...
    // Getters for death message items and messages
    public Map<String, DeathMessageItem> getDeathMessageItems() { return new HashMap<>(deathMessageItems); }
    ...
    public DeathMessageItem getDeathMessageItem(String key) {
        return deathMessageItems.get(key);
    }
    ...
    public List<DeathMessageItem> getSortedDeathMessagesByRarity() {
        Map<String, Integer> rarityOrder = new HashMap<>();
        rarityOrder.put("COMMON", 1);
        rarityOrder.put("EPIC", 2);
        rarityOrder.put("LEGENDARY", 3);
        
        List<DeathMessageItem> sortedList = new ArrayList<>(deathMessageItems.values());
    ...
    // Inner classes
    public static class DeathMessageItem {
        ...
        public DeathMessageItem(String key, String name, String message, int price, String rarity, 
