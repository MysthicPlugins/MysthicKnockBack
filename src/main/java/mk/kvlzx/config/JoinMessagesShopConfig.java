...
public class JoinMessagesShopConfig {
    ...
    private List<Integer> joinMessageSlots;
    private String joinMessageTitle;
    private List<String> joinMessageLore;
    ...
    // Join message items
    private Map<String, JoinMessageItem> joinMessageItems;
    ...
    public JoinMessagesShopConfig(MysthicKnockBack plugin) {
        this.plugin = plugin;
        configFile = new CustomConfig("join-messages-shop.yml", "config/menus", plugin);
    ...
    public void loadConfig() {
        ...
        // Join messages configuration
        joinMessageSlots = config.getIntegerList("menu.items.join_messages.slots");
        joinMessageTitle = config.getString("menu.items.join_messages.title");
        joinMessageLore = config.getStringList("menu.items.join_messages.lore");
        enchantedIfSelected = config.getBoolean("menu.items.join_messages.enchanted_if_selected");
        hideEnchants = config.getBoolean("menu.items.join_messages.hide_enchants");
        ...
        // Load join message items
        loadJoinMessageItems(config);
    ...
    private void loadRarityMaterials(FileConfiguration config) {
        ...
        ConfigurationSection materialsSection = config.getConfigurationSection("menu.items.join_messages.materials");
            ...
            for (String rarity : materialsSection.getKeys(false)) {
                ...
                if (raritySection != null) {
                    String selected = validateAndGetMaterial(config, 
                        "menu.items.join_messages.materials." + rarity + ".selected", "ENCHANTED_BOOK");
                    String unselected = validateAndGetMaterial(config, 
                        "menu.items.join_messages.materials." + rarity + ".unselected", "PAPER");
        ...
        if (!rarityMaterials.containsKey("COMMON")) {
            rarityMaterials.put("COMMON", new MaterialPair("ENCHANTED_BOOK", "PAPER"));
        }
        if (!rarityMaterials.containsKey("EPIC")) {
            rarityMaterials.put("EPIC", new MaterialPair("ENCHANTED_BOOK", "WRITTEN_BOOK"));
        }
        if (!rarityMaterials.containsKey("LEGENDARY")) {
            rarityMaterials.put("LEGENDARY", new MaterialPair("ENCHANTED_BOOK", "BOOK_AND_QUILL"));
        }
    }
    
    private void loadJoinMessageItems(FileConfiguration config) {
        joinMessageItems = new HashMap<>();
        ConfigurationSection joinMessageItemsSection = config.getConfigurationSection("join_message_items");
        
        if (joinMessageItemsSection != null) {
            for (String key : joinMessageItemsSection.getKeys(false)) {
                ConfigurationSection messageSection = joinMessageItemsSection.getConfigurationSection(key);
                if (messageSection != null) {
                    JoinMessageItem messageItem = new JoinMessageItem(
                        key,
                    ...
                    joinMessageItems.put(key, messageItem);
                }
            }
        }
    }
                        ...
                        if (materialName != null && !materialName.trim().isEmpty()) {
                            try {
                                ...
                                plugin.getLogger().severe("Invalid material detected: '" + materialName + 
                                    "' at 'menu.items.join_messages.materials." + rarity + "." + key + "'");
                                allValid = false;
                            }
                        }
                    }
    ...
    public void reload() {
        ...
        if (!validateAllMaterials()) {
            plugin.getLogger().warning("Invalid materials detected in join messages menu configuration. The menu will be loaded with default values for problematic materials.");
        }
    ...
    // Getters for join messages configuration
    public List<Integer> getJoinMessageSlots() { return new ArrayList<>(joinMessageSlots); }
    public String getJoinMessageTitle() { return joinMessageTitle; }
    public List<String> getJoinMessageLore() { return new ArrayList<>(joinMessageLore); }
    ...
    // Getters for join message items and messages
    public Map<String, JoinMessageItem> getJoinMessageItems() { return new HashMap<>(joinMessageItems); }
    ...
    public JoinMessageItem getJoinMessageItem(String key) {
        return joinMessageItems.get(key);
    }
    ...
    public List<JoinMessageItem> getSortedJoinMessagesByRarity() {
        Map<String, Integer> rarityOrder = new HashMap<>();
        rarityOrder.put("COMMON", 1);
        rarityOrder.put("EPIC", 2);
        rarityOrder.put("LEGENDARY", 3);
        
        List<JoinMessageItem> sortedList = new ArrayList<>(joinMessageItems.values());
    ...
    // Inner classes
    public static class JoinMessageItem {
        ...
        public JoinMessageItem(String key, String name, String message, int price, 
                                String rarity, String rarityColor, String description) {
            this.key = key;
            this.name = name;
