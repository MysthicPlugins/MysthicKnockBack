...
public class DeathSoundsShopConfig {
    ...
    // Death sounds configuration
    private List<Integer> deathSoundSlots;
    private String deathSoundTitle;
    private List<String> deathSoundLore;
    ...
    // Death sound items
    private Map<String, DeathSoundItem> deathSoundItems;
    ...
    public DeathSoundsShopConfig(MysthicKnockBack plugin) {
        this.plugin = plugin;
        configFile = new CustomConfig("death-sounds-shop.yml", "config/menus", plugin);
    ...
    public void loadConfig() {
        ...
        // Death sounds configuration
        deathSoundSlots = config.getIntegerList("menu.items.death_sounds.slots");
        deathSoundTitle = config.getString("menu.items.death_sounds.title");
        deathSoundLore = config.getStringList("menu.items.death_sounds.lore");
        enchantedIfSelected = config.getBoolean("menu.items.death_sounds.enchanted_if_selected");
        hideEnchants = config.getBoolean("menu.items.death_sounds.hide_enchants");
        ...
        // Load death sound items
        loadDeathSoundItems(config);
    ...
    private void loadRarityMaterials(FileConfiguration config) {
        ...
        ConfigurationSection materialsSection = config.getConfigurationSection("menu.items.death_sounds.materials");
            ...
            for (String rarity : materialsSection.getKeys(false)) {
                ...
                if (raritySection != null) {
                    String selected = validateAndGetMaterial(config, 
                        "menu.items.death_sounds.materials." + rarity + ".selected", "JUKEBOX");
                    String unselected = validateAndGetMaterial(config, 
                        "menu.items.death_sounds.materials." + rarity + ".unselected", "NOTE_BLOCK");
        ...
        if (!rarityMaterials.containsKey("EPIC")) {
            rarityMaterials.put("EPIC", new MaterialPair("JUKEBOX", "RECORD_3"));
        }
    ...
    private void loadDeathSoundItems(FileConfiguration config) {
        deathSoundItems = new HashMap<>();
        ConfigurationSection deathSoundItemsSection = config.getConfigurationSection("death_sound_items");
        
        if (deathSoundItemsSection != null) {
            for (String key : deathSoundItemsSection.getKeys(false)) {
                ConfigurationSection soundSection = deathSoundItemsSection.getConfigurationSection(key);
                if (soundSection != null) {
                    // Validate sound
                    String soundName = soundSection.getString("sound", "");
                    Sound sound = validateAndGetSound(soundName);
                    
                    DeathSoundItem soundItem = new DeathSoundItem(
                        key,
                    ...
                    deathSoundItems.put(key, soundItem);
                }
            }
        }
    }
    ...
    private Sound validateAndGetSound(String soundName) {
        if (soundName == null || soundName.trim().isEmpty()) {
            plugin.getLogger().warning("Sound name is empty. Using default sound: HURT_FLESH");
        ...
        try {
            ...
            plugin.getLogger().warning("Invalid sound '" + soundName + "'. Using default sound: HURT_FLESH");
            return Sound.HURT_FLESH;
        }
    }
                        ...
                        if (materialName != null && !materialName.trim().isEmpty()) {
                            try {
                                ...
                                plugin.getLogger().severe("Invalid material detected: '" + materialName + 
                                    "' at 'menu.items.death_sounds.materials." + rarity + "." + key + "'");
                                allValid = false;
                            }
                        }
                    }
    ...
    public void reload() {
        ...
        if (!validateAllMaterials()) {
            plugin.getLogger().warning("Invalid materials detected in death sounds menu configuration. The menu will be loaded with default values for problematic materials.");
        }
    ...
    // Getters for death sounds configuration
    public List<Integer> getDeathSoundSlots() { return new ArrayList<>(deathSoundSlots); }
    public String getDeathSoundTitle() { return deathSoundTitle; }
    public List<String> getDeathSoundLore() { return new ArrayList<>(deathSoundLore); }
    ...
    // Getters for death sound items and messages
    public Map<String, DeathSoundItem> getDeathSoundItems() { return new HashMap<>(deathSoundItems); }
    ...
    public DeathSoundItem getDeathSoundItem(String key) {
        return deathSoundItems.get(key);
    }
    ...
    public List<DeathSoundItem> getSortedDeathSoundsByRarity() {
        Map<String, Integer> rarityOrder = new HashMap<>();
        rarityOrder.put("COMMON", 1);
        rarityOrder.put("EPIC", 2);
        rarityOrder.put("LEGENDARY", 3);
        
        List<DeathSoundItem> sortedList = new ArrayList<>(deathSoundItems.values());
    ...
    // Inner classes
    public static class DeathSoundItem {
        ...
        public DeathSoundItem(String key, String name, Sound sound, float volume, float pitch, 
