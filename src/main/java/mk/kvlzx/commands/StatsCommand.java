...
public class StatsCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 1 && !isOperationCommand(args[0])) {
            ...
            if (target == null) {
                sender.sendMessage(MessageUtils.getColor(MysthicKnockBack.prefix + messages.getStatsNotFound()));
                return true;
            }
        ...
        if (!sender.hasPermission("kvknockback.stats.admin")) {
            sender.sendMessage(MessageUtils.getColor(MysthicKnockBack.prefix + messages.getStatsNoPermission()));
            return true;
        }

        if (args[0].equalsIgnoreCase("resetall")) {
            resetAllStats();
            sender.sendMessage(MessageUtils.getColor(MysthicKnockBack.prefix + messages.getStatsResetAllSuccess()));
            return true;
        }

        if (args[0].equalsIgnoreCase("reset")) {
            if (args.length < 2) {
                sender.sendMessage(MessageUtils.getColor(MysthicKnockBack.prefix + messages.getStatsAdminUsage()));
                return true;
            }
            ...
            sender.sendMessage(MessageUtils.getColor(MysthicKnockBack.prefix + messages.getStatsResetSuccess().replace("%player%", target.getName())));
            return true;
        }
    ...
    private void handleStatModification(CommandSender sender, String[] args) {
        ...
        try {
            ...
            sender.sendMessage(MessageUtils.getColor(MysthicKnockBack.prefix + messages.getStatsAmountError()));
            return;
        }
        ...
        switch (stat) {
            ...
            case "coins":
                ...
                break;
            default:
                sender.sendMessage(MessageUtils.getColor(MysthicKnockBack.prefix + messages.getStatsInvalidStat()));
                return;
        }

        String operationName = operation.equals("set") ? "set" : 
                                operation.equals("add") ? "added" : "removed";
        sender.sendMessage(MessageUtils.getColor(MysthicKnockBack.prefix + messages.getStatsStatUpdated()
